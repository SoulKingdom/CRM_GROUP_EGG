								...
								<tbody>
									<c:forEach items="${linkList}" var="link">
										<tr data-key="4">
											...
											<td>${link.phoneNum }</td>
											<td>${link.email }</td>
											<td><a href="mailto:xxxx@xxx.com">${link.QQ}</a></td>
											<td>${link.createTime}</td>
											<td>${link.updateTime}</td>
											<td></td>
											<td><button class="btn btn-success btn-sm" title="查看"
													aria-label="查看" onclick="examine('${link.linkManId}')">查看</button>
												<button class="btn btn-warning btn-sm" title="更新"
													aria-label="更新"
													onclick="updateLinkMan('${link.linkManId}')">修改</button></td>
										</tr>
									</c:forEach>
									<%--  <tr data-key="4">
                                    <td>
...
function examine(linkManId){
location.href='LinkManServlet?method=selectByPrimaryKey&type=1&linkManId='+linkManId;
} 


function updateLinkMan(linkManId){
location.href='LinkManServlet?method=selectByPrimaryKey&linkManId='+linkManId;
}
