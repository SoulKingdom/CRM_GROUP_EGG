							...
							<div class="form-group field-business-owner_name">
								...
								<div class="col-sm-8">
									<input type="text" id="employeeName"
										...
										readonly="readonly" onclick="selectEmployee()"
										placeholder="点击选择负责人"> <input type="hidden"
										id="employeeId" />
								</div>
							...
							<div class="form-group field-business-selectcustomer required">
								...
								<div class="col-sm-8">
									<input type="text" id="business-selectcustomer"
										...
										onclick="selectCustomer()" /> <input type="hidden"
										id="customerId" />
							...
							<div class="form-group field-business-type required">
								...
								<div class="col-sm-8">
									<select id="business-type" class="form-control"
										...
										<c:forEach items="${businessTypeList}" var="businessTypeList">
											<option value="${businessTypeList.typeId}">${businessTypeList.typeName}</option>
										</c:forEach>
								...
								<div class="col-sm-8">
									<select id="business-status" class="form-control"
										...
										<c:forEach items="${businessList}" var="businessList">
											<option value="1">${businessList.status}</option>
										</c:forEach>
							...
							<div class="form-group field-business-origin required">
								...
								<div class="col-sm-8">
									<select id="business-origin" class="form-control"
										...
										<c:forEach items="${businessSourceList}"
											var="businessSourceList">
											<option value="${businessSourceList.sourceId}">${businessSourceList.sourceName}</option>
										</c:forEach>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<table id="exampleCustomer" class="table">
						...
						<tbody>
							<c:forEach items="${principalVoList}" var="principalVoList">
								<tr>
									<td><input type="radio" name="radioEmployeeName"
										style="opacity: 1; margin-top: -6px"
										id="${principalVoList.employeeId}" /></td>
									<td id="${principalVoList.employeeId}_emp">${principalVoList.employeeName}
									</td>
									<td>${principalVoList.departmentName}</td>
									<td>${principalVoList.positionName}</td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
...
<script type="text/javascript">
	...
	function addBuness() {
		...
		if (data.employeeId!=null&&data.employeeId!=""&&data.employeeId!="undefined") {
