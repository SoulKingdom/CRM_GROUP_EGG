...
<body class="navbar-top">
											...
											<td>
												<div>
													<button type="button" name="updatebtn"
														...
														id="${customerVo.customerId }_ct"
														onclick="checkCustomer('${customerVo.customerId }')">查看</button>
													<button type="button" name="updatebtn"
														class="btn btn-warning btn-sm"
														id="${customerVo.customerId }_xg"
														onclick="updateCustomer('${customerVo.customerId }')">修改</button>
												</div>
											</td>
										</tr>
									</c:forEach>
	...
	<script type="text/javascript">
		...
		function updateCustomer(id) {
			...
			location.href = 'CustomerServlet?method=selectByPrimaryKey&type=1&customerId='+customerId;
		}
			...
			if ($("#searchContent").css('display') == "inline-block") {
				...
				if (text != null && text != "undefined" && text != "") {
					if (selectTest == "客户名称") {
						$("#actionFormSo").attr(
								"action",
								"CustomerServlet?method=getSomeCustomer&text="+text/*encodeURI(encodeURI(text))*/+ "&type=1 ");
						$("#actionFormSo").submit();
				...
				$("#actionFormSo").attr(
						"action",
						"CustomerServlet?method=getAllCustomer");
				$("#actionFormSo").submit();
			}
		}
	</script>
