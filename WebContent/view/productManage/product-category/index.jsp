		...
		<div class="container-fluid main-content">
			<div class="widget-container fluid-height clearfix">
				<div class="heading clearfix">
					<i class="icon-table"></i> 产品类别 <a
						class="weiBoxs-dialog btn btn-sm btn-primary-outline pull-right"
						href="product-category/create.html" dialog-title="添加分类"><i
						class="icon-plus"></i> 添加分类</a>
				</div>
						...
						<tbody>
							<c:forEach items="${productTypeList }" var="productType">
								<tr>
									...
									<td>${productType.typeDescription }</td>
									<td class="text-right" nowrap><a
										class="btn btn-warning btn-xs weiBoxs-dialog"
										href="productType.do?method=toCreateUl&typeId=${productType.typeId }&typeName=${productType.typeName }"
										dialog-title="修改类别"><i class="glyphicon glyphicon-plus"></i>
											添加子类</a> <a class="btn btn-primary btn-xs weiBoxs-dialog"
										href="product-category/update.html?id=1" dialog-title="修改类别"><i
											class="fa fa-edit"></i> 修改</a> <a class="btn btn-default btn-xs"
										href="#" disabled><i class="glyphicon glyphicon-trash"></i>
											删除</a></td>
								</tr>
								<c:forEach items="${productType.productTypeList }"
									var="productTypes">
									<tr>
										<td>${productTypes.typeId }</td>
										<td>&nbsp;&nbsp;|-&nbsp;${productTypes.typeName }</td>
										<td>${productTypes.typeDescription }</td>
										<td class="text-right" nowrap><a
											class="btn btn-primary btn-xs weiBoxs-dialog"
											href="product-category/update.html?id=6" dialog-title="修改类别"><i
												class="fa fa-edit"></i> 修改</a> <a
											class="btn btn-danger btn-xs btn-delete-dialog"
											href="productType.do?method=remove&typeId=${productTypes.typeId }"
											title="删除" dialog-tip="确定要删除该类别吗？删除后不可恢复哦！"><i
