...
<script type="text/javascript">
	...
	function product_calculate(num) {
		...
		//判断不为空
		if(vproAmount!="" && vproduct_retailPrice !="" && product_discount_rate_1 !=""){
			//折扣额
			vproduct_discount_1=Number(vproAmount)*Number(vproduct_retailPrice)*Number(product_discount_rate_1)*Number(0.01);
			$("#product_discount_1").val(vproduct_discount_1);
		}
		
		if(vproAmount!="" && vproduct_retailPrice !="" && vproduct_tax_rate_1!=""){
			//税额
			varProduct_rate_money = Number(vproAmount)*Number(vproduct_retailPrice)*Number(vproduct_tax_rate_1)*Number(0.01);
			$("#product_tax_price_1").val(varProduct_rate_money);
			//税前
			$("#product_no_tax_price_1").val(Number(vproAmount)*Number(vproduct_retailPrice));
			//税后
			$("#product_prime_price_1").val(Number(vproAmount)*Number(vproduct_retailPrice)-Number(varProduct_rate_money));
			$("#totalMoney").val(Number(vproAmount)*Number(vproduct_retailPrice)-Number(varProduct_rate_money));
		}
		totalMoney
	}
</script>
		...
		<div class="container-fluid main-content">
			<div class="widget-container fluid-height clearfix">
				<div class="heading clearfix">
					<i class="icon-reorder"></i> 新增退货单 <a href="javascript:;"
						class="pull-right" onclick="history.go(-1);"><i
						class="icon-reply"></i></a>
				</div>
				<div class="widget-content padded clearfix">
					<form id="billForm" class="form-horizontal" action="sales.do"
						method="get">
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0"
								...
								<tr>
									<td class="form-inline"><label>客户：</label> <input
										...
										value=""> <label style="margin-left: 10px;">销售日期：</label>
										...
										<div style="float: right;">
											...
											<input type="hidden" name="status" value="2" /> <input
												type="hidden" name="orederType" value="3" />
										</div></td>
								</tr>
							</table>
							<div class="row-fluid">
